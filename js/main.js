...
class MainApp {
    ...
    setupContentBasedLoading() {
        ...
        const chartsSection = document.querySelector('[data-module="charts"], .charts-section, .data-visualization');
        if (chartsSection) {
            chartsSection.setAttribute('data-module', 'charts');
        }

        // Load forms module when forms are visible
        const forms = document.querySelectorAll('form[data-lazy="true"], .form-section[data-lazy="true"]');
        forms.forEach(form => {
    ...
    addLazyModuleAttributes() {
        ...
        const importantButtons = document.querySelectorAll('.btn-primary, .cta-button, [data-track="true"]');
    ...
    showModuleLoadError(moduleName) {
        ...
        const message = errorMessages[moduleName] || `${moduleName} features are currently unavailable.`;
