...
const positionsData = [
  {
    ...
    description:
      'Comprehensive guide to software development roles, from junior developers to senior architects. Learn about required skills, technologies, and career progression.',
  ...
  },
  {
    ...
    subcategory: 'product',
    description:
      'Deep dive into product management roles, including responsibilities, skills, tools, and career paths from associate to senior product management positions.',
    url: 'product-manager/',
    featured: true,
    skills: [
      'Product Strategy',
  ...
  },
  {
    ...
    description:
      'Specialized guide for AI and machine learning product management, covering unique challenges, required knowledge, and emerging opportunities in AI products.',
  ...
  },
  {
    ...
    subcategory: 'data',
    description:
      'Comprehensive guide to data science roles, covering statistical analysis, machine learning, data visualization, and business intelligence.',
    url: 'data-scientist/',
    featured: false,
  ...
  },
  {
    ...
    description:
      'Complete guide to UX design roles, including user research, interaction design, prototyping, and design thinking methodologies.',
    url: 'ux-designer/',
    featured: false,
  ...
  },
  {
    ...
    description:
      'Essential guide to DevOps roles, covering CI/CD pipelines, cloud infrastructure, containerization, and infrastructure as code.',
    url: 'devops-engineer/',
    featured: false,
...
class PositionManager {
  ...
  applyFilters() {
    ...
    if (this.filters.category) {
      filtered = filtered.filter(position => position.category === this.filters.category);
    }

    // 工作类型筛选
    if (this.filters.type) {
      filtered = filtered.filter(position => position.type === this.filters.type);
    }

    // 远程工作筛选
    if (this.filters.remote !== null) {
      filtered = filtered.filter(position => position.remote === this.filters.remote);
    }
    ...
    if (this.filters.minSalary !== null) {
      filtered = filtered.filter(position => position.salary.min >= this.filters.minSalary);
    }

    if (this.filters.maxSalary !== null) {
      filtered = filtered.filter(position => position.salary.max <= this.filters.maxSalary);
    }
  ...
  getFeaturedPositions(limit = 3) {
    return this.positions.filter(p => p.featured).slice(0, limit);
  }
